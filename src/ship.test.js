...
describe('ship class has working methods and valid properties', () => {
  ...
  test('ship class hit() function adds 1 to current hits', () => {
    const oldHits = oneObj.hits;
    oneObj.hit();
    expect(oneObj.hits).toBe(oldHits + 1);
  });

  test('ship class isSunk() function updates sink property to false if length > hits', () => {
		oneObj.hits = 2
  	oneObj.isSunk()
    expect(oneObj.sink).toBe(false);
  });

  test('ship class isSunk() function updates sink property to true if length < hits', () => {
		oneObj.hits = 4
  	oneObj.isSunk()
